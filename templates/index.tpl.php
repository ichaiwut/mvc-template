...
<div class="team-view">
	...
	<form action="?action=index" method="GET" class="team-select right">
		...
		<select name="employee-site" id="employeeSite">
            ...
            <?php
				function siteparentx($value,$dot,$site){
						mysql_connect("localhost","sapphire","sprd!@#$%");
						mysql_query("SET NAMES utf8");
						$dots = $dot.'---';
						$sqlsite = mysql_query("SELECT site_name,site_id FROM faceaccess.faceacc_site WHERE site_parent=$value AND site_hidden!='1' ORDER BY site_name ASC");
						while( $rowPres = mysql_fetch_assoc($sqlsite)){
							$chpar = mysql_num_rows(mysql_query("SELECT * FROM  faceaccess.faceacc_site WHERE site_parent=$rowPres[site_id]"));
							$sitepar .= '<option value="'.$rowPres['site_id'].'" ';
							if($rowPres['site_id']==$site){ $sitepar .= "selected"; }
							if($chpar>0){ $sitepar .='disabled="disabled"'; }
							$sitepar .= '>'.$dots.$rowPres['site_name'].'</option>';
							$sitepar .= siteparentx($rowPres['site_id'],$dots,$site);
						}
						return $sitepar;
					}
					...
					mysql_connect("localhost","sapphire","sprd!@#$%");
					mysql_query("SET NAMES utf8");
					$sqlsite = mysql_query("SELECT site_name,site_id FROM faceaccess.faceacc_site WHERE site_parent=0  AND site_hidden!='1' ORDER BY site_name ASC");
					while( $rowPre = mysql_fetch_assoc($sqlsite)){
					?>
					<option value="<?=$rowPre['site_id']?>" disabled="disabled">
                      <?=$rowPre['site_name']?>
                      </option>
                    <?
						echo siteparentx($rowPre['site_id'],'',$_GET['employee-site']);
				  }
				  ?>
		...
		<tbody>
			...
			<?php foreach ( $people as $value ) :?>
				<tr>
					...
					<td>
						<a href="?action=viewDetail&id=<?php echo $value['officer_id']; ?>&startDate=<?php echo $startTime;?>&endDate=<?php echo $endTime;?>" title="<?php echo $value['prename_th'] . $value['firstname'] . ' ' . $value['surname']; ?>">
